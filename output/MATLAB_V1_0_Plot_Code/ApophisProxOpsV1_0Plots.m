...
% NSTGRO
% Mothership-Daughtership Formation Plotting for MPC V1.0

% Dependencies: ApophisProxOpsV1_0Plots.m, MATLAB_mats/...,
...
figure()
axis equal
scatter3(verts(:,1), verts(:,2), verts(:,3), [], [0.3 0.3 0.3], 'filled')
...
zlabel("Z (km)")
hold on
plot3(s_hist_m(1, :), s_hist_m(2, :), s_hist_m(3, :), "o-", 'MarkerSize', 3, 'Color', color1)
plot3(s_hist_1(1, :), s_hist_1(2, :), s_hist_1(3, :), "square-", 'MarkerSize', 3, 'Color', color2)
plot3(s_hist_2(1, :), s_hist_2(2, :), s_hist_2(3, :), "+-", 'MarkerSize', 3, 'Color', color3)
...
[~, max_accel_m_x] = max(true_accel_hist_m(1,:));
[~, max_accel_m_y] = max(true_accel_hist_m(2,:));
[~, max_accel_m_z] = max(true_accel_hist_m(3,:));
[~, max_accel_1_x] = max(true_accel_hist_1(1,:));
[~, max_accel_1_y] = max(true_accel_hist_1(2,:));
[~, max_accel_1_z] = max(true_accel_hist_1(3,:));
[~, max_accel_2_x] = max(true_accel_hist_2(1,:));
[~, max_accel_2_y] = max(true_accel_hist_2(2,:));
[~, max_accel_2_z] = max(true_accel_hist_2(3,:));
...
% Cost components
figure()
plot(epochs, pos_cost_hist, "o-", 'MarkerSize', 3)
hold on
plot(epochs, dot_cost_hist, "+-", 'MarkerSize', 3)
plot(epochs, dv_cost_1_hist, ">-", 'MarkerSize', 3)
plot(epochs, dv_cost_2_hist, "diamond-", 'MarkerSize', 3)
plot(epochs, e_cost_hist, "square-", 'MarkerSize', 3)
plot(epochs, a_cost_hist, "x-", 'MarkerSize', 3)
plot(epochs, i_cost_hist, "pentagram-", 'MarkerSize', 3)
hold off
xlabel('Epochs (\Deltat)')
ylabel('Cost')
...
figure()
plot(1:(duration-1), bistatic_angle_true_hist(1:end-1), "+-", 'MarkerSize', 3, "Color", color1)
hold on
plot(1:(duration-1), bistatic_angle_hat_hist(1:end-1), "square-", 'MarkerSize', 3, "Color", color3)
hold off
xlabel('Epochs (\Deltat)')
...
spline_x = griddedInterpolant(1:length(s_sun_to_apophis(1,:)), s_sun_to_apophis(1,:), 'spline');
spline_y = griddedInterpolant(1:length(s_sun_to_apophis(2,:)), s_sun_to_apophis(2,:), 'spline');
spline_z = griddedInterpolant(1:length(s_sun_to_apophis(3,:)), s_sun_to_apophis(3,:), 'spline');

offset_1_hist = zeros(1,duration);
offset_2_hist = zeros(1,duration);
[a_hist_actual_1, e_hist_actual_1, i_hist_actual_1, raan_hist_actual_1, aop_hist_actual_1, r_rot_1, v_rot_1] = compute_elements_over_time(mu_apophis, s_hist_1, spline_x, spline_y, spline_z, offset_1, offset_1_hist);
[a_hist_actual_2, e_hist_actual_2, i_hist_actual_2, raan_hist_actual_2, aop_hist_actual_2, r_rot_2, v_rot_2] = compute_elements_over_time(mu_apophis, s_hist_2, spline_x, spline_y, spline_z, offset_2, offset_2_hist);
